...
- **`backend/app/plugins/tushare/`** — Tushare Pro 官方 HTTP 数据源(runtime: none, 直接 POST `api.tushare.pro`, 不依赖 `tushare` SDK / pandas)
  ...
  - `provider.py` — Provider 实现(实测单位换算: 日K `amount` 千元→元且 `vol` 已是手; 分钟 `vol` 股→手且 `amount` 已是元; 财务 `grossprofit_margin` 是率、`gross_margin` 是额不可混用; datetime 为北京墙钟; 分批软失败 + 进度回调覆盖空批; 设置页试拉先直连预检以便暴露积分/权限错误)
  - `tests/test_tushare_provider.py` — 契约测试(不依赖网络与 Key, 假客户端注入), 覆盖单位口径、复权因子换算、行数预算分批、财务多表映射与软失败、能力声明与 Key 语义

...
常见误区:在自定义源 YAML 里声明 `full_minute` **只是让该源进入候选**, 生效还需在设置页把「全量分钟」这一行点选到它(每个能力独立路由, 不会自动切);若侧栏数据源徽标显示「全量分钟 未接入」而候选里已有源, 那就是**路由还指在别处**(例如默认 TickFlow 但档位不足),点徽标右侧的候选标签即可切换。修 YAML 后还需点「重新加载」(或重启后端)让配置进内存。
